...
var keyGeoObject = new THREE.Object3D();
...
mtlLoader.load( "dataset/Key_B_02.mtl", function( materials ) {
    materials.preload();
    var objLoader = new THREE.OBJLoader();
    objLoader.setMaterials( materials );
    objLoader.load("dataset/Key_B_02.obj", function ( object ) {
        object.rotation.y = 180* Math.PI / 180;
        object.scale.x = 5;
        object.scale.y = 5;
        object.scale.z = 5;
...
var keyGeoEntity = new Argon.Cesium.Entity({
    name: "I have a key",
    position: Cartesian3.ZERO,
    orientation: Cesium.Quaternion.IDENTITY
});
...
app.updateEvent.addEventListener(function (frame) {
    ...
    if (!keyInit) {
        ...
        // set the box's position to 10 meters away from the user.
        // First, clone the userPose postion, and add 10 to the X
        var keyPos = userPose.position.clone();
        keyPos.x -= 10;
        // set the value of the box Entity to this local position, by
        // specifying the frame of reference to our local frame
        keyGeoEntity.position.setValue(keyPos, defaultFrame);
        // orient the box according to the local world frame
        keyGeoEntity.orientation.setValue(Cesium.Quaternion.IDENTITY);
        // now, we want to move the box's coordinates to the FIXED frame, so
        // the box doesn't move if the local coordinate system origin changes.
        if (Argon.convertEntityReferenceFrame(keyGeoEntity, frame.time, ReferenceFrame.FIXED)) {
            scene.add(keyGeoObject);
            keyInit = true;
        }
    }
    // get the local coordinates of the local box, and set the THREE object
    var keyPose = app.context.getEntityPose(keyGeoEntity);
    keyGeoObject.position.copy(keyPose.position);
    keyGeoObject.quaternion.copy(keyPose.orientation);
    // rotate the boxes at a constant speed, independent of frame rates     
    // to make it a little less boring
    key.rotateY(3 * frame.deltaTime / 10000);
});
